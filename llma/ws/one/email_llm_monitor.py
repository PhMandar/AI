...
def fetch_all_emails_after_date(sender_email, after_utc):
    print("\nStarted reading mail box\n")
    ...
    for eid in reversed(email_ids):
        ...
        new_emails.append((email_date, body))

    imap.logout()
    return sorted(new_emails, key=lambda x: x[0])
...
if __name__ == "__main__":
    print(Fore.CYAN + "\n📧 Gmail Monitor with LLM and Excel Logging (Running until Ctrl+C)\n")

    last_ts = read_last_timestamp() or FILTER_START_DATE
    print(Fore.BLUE + f"\n🕒 Starting from: {last_ts.isoformat()}\n")

    try:
        while True:
            print("\n Fetching mails now")
            emails = fetch_all_emails_after_date(TARGET_SENDER, last_ts)
            for email_time, email_body in emails:
                print(Fore.GREEN + f"📨 Processing email received at {email_time.isoformat()}")
